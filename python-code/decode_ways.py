...
class Solution:
    ...
    def _do_decode(self, s: str, index: int) -> int:
        ...
        using_first_option: int = (
            self._do_decode(s[1:], index + 1) if self._is_valid(first_way) else 0
        )

        using_second_option: int = (
            self._do_decode(s[2:], index + 2) if self._is_valid(second_way) else 0
        )
