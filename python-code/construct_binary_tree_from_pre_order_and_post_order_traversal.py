...
class Solution:
    def construct_from_pre_order_and_post_order(
        ...
        root.left = self.construct_from_pre_order_and_post_order(
            preorder[1 : 1 + left_subtree_size], postorder[:left_subtree_size]
        )
        root.right = self.construct_from_pre_order_and_post_order(
