...
class ImageDownloaderThreadPoolExecutor:
    ...
    def __init__(
        self, sources_dir: Path, target_dir: Path, number_of_workers: int = 10
    ):
    ...
    def create_single_downloader_task(self, json_file: Path):
        with open(json_file) as f:
            if not self.is_empty(json_file):
                ...
                print(
                    f"[thread_id: {get_ident()}] {json_file} is empty. Skipping processing"
                )

    def initialize_all_downloader_tasks(self):
        with ThreadPoolExecutor(max_workers=self.number_of_workers) as tpx:
            self.task_futures = [
                tpx.submit(self.create_single_downloader_task, json_file=json_file)
    ...
    def wait_for_completion_and_stop(self):
        if not self.task_futures:
            print(
                "No tasks initialized. Maybe you have not invoked the create_all_downloader_tasks yet"
            )
            return
...
if __name__ == "__main__":
    ...
    image_downloader: ImageDownloaderThreadPoolExecutor = (
        ImageDownloaderThreadPoolExecutor(a_sources_dir, a_target_dir)
    )
