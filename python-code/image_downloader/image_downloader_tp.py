...
class ImageDownloaderThreadPool:
    ...
    def download(self):
        with ThreadPool(self.number_of_workers) as pool:
            pool.map(
                self.create_single_downloader_task, self.sources_dir.glob("**/*.json")
            )
...
if __name__ == "__main__":
    ...
    image_downloader: ImageDownloaderThreadPool = ImageDownloaderThreadPool(
        a_sources_dir, a_target_dir
    )
