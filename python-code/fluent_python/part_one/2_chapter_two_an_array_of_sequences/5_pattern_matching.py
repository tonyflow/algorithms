...
def evaluate(expression: Expression, an_environment: Environment) -> Any:
    match expression:
        case ["quote", x]:
            return x
        case ["if", test, consequence, alternative]:
            if evaluate(test, an_environment):
                return evaluate(consequence, an_environment)
            else:
                return evaluate(alternative, an_environment)
        case ["lambda", [*params], *body] if body:
            print(
                f"Identified a lambda expression with params {params} and body {body}"
            )
        case ["define", Symbol() as symbol, *value_exp] if value_exp:
            print(
                f"Identified a function definition with symbols {str(symbol)} and value expression {value_exp}"
            )
        case ["define", [Symbol() as symbol, *params], *body]:
            print(
                f"Alternative function definition with symbols {symbol} and others {params} and body {body}"
            )
...
if __name__ == "__main__":
    ...
    evaluate(
        ["define", [Symbol("function_name"), "a", "b", "c"], [["n"], ["*", "n", 2]]],
        environment,
    )

    evaluate(
        ["define", Symbol("double"), ["lambda", ["n"], ["*", "n", 2]]], environment
    )

    evaluate(["lambda", ["x"], ["*", "x", 2]], environment)
