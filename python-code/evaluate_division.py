...
class Solution:
    ...
    def calcEquation(
        ...
        visited: Set[str] = set()
        self.directed_weighted_graph: Dict[str, Dict[str, float]] = self.build_graph(
            equations, values
        )
        ...
        for i, query in enumerate(queries):
            ...
            if (
                start not in self.directed_weighted_graph
                or end not in self.directed_weighted_graph
            ):
    ...
    def traverse(self, start: str, end: str, visited: Set[str], partial_result: float):
        visited.add(start)
        if start == end:
    ...
    def build_graph(self, equations: List[List[str]], values: List[float]):
        ...
        for i, equation in enumerate(equations):
            ...
            if second_operand in directed_weighted_graph:
                ...
                directed_weighted_graph[second_operand] = {
                    first_operand: 1.0 / values[i]
                }
